...
void led_init(void)
{
    ...
    /* GPIO配置 */
    GPIO_InitStructure.GPIO_Pin = LED_RF_PIN | LED_LB_PIN | LED_RB_PIN;
    GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
    GPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_OD;
    GPIO_Init(LED_PORT1, &GPIO_InitStructure);
    
    GPIO_InitStructure.GPIO_Pin = LED_LF_PIN;
    GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
    GPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_OD;
...
void led_set(which_led_e led, led_status_e status)
{
    switch (led)
    {
        case LED_LF:
            if (status == ON)
                GPIO_ResetBits(LED_PORT2, LED_LF_PIN);
            else if (status == OFF)
                GPIO_SetBits(LED_PORT2, LED_LF_PIN);
            else if (status == TOGGLE)
                GPIO_WriteBit(LED_PORT2, LED_LF_PIN, (BitAction)(1 - GPIO_ReadOutputDataBit(LED_PORT2, LED_LF_PIN)));
        break;
        ...
        case LED_LB:
            if (status == ON)
                GPIO_ResetBits(LED_PORT1, LED_LB_PIN);
            else if (status == OFF)
                GPIO_SetBits(LED_PORT1, LED_LB_PIN);
            else if (status == TOGGLE)
                GPIO_WriteBit(LED_PORT1, LED_LB_PIN, (BitAction)(1 - GPIO_ReadOutputDataBit(LED_PORT1, LED_LB_PIN)));
        break;
